...
sequence therefore remains as a draft (pQBR11d) based on joining a path
between the two contigs assembled from the WGS assembly and Illumina
unmapped reads described above, with reference to the canonical Group I
plasmid pQBR103.

#### pQBR43 (Group I)

This sequence didn’t close owing to a duplicate sequence in the
chromosome. The plasmid-specific sequence is bordered by Tn6290 copies.
Two possibilities: both the chromosome and the plasmid contain the same
duplicated element but remain separate replicons, or the plasmid has
integrated into the chromosome, flanked by copies of the duplication.

Resolving this is complex, as the duplicated element is 42 kb long and
there are insufficient reads overlapping the junction. Tn6290 is present
at ~100x coverage, whereas the chromosome at ~35x and the plasmid at
~80x, suggesting one copy of the transposon on the plasmid and one
remaining on the chromosome.

The plasmid-specific contig was extracted for subsequent analysis.
...
and the sequence assembled as a single contig including plasmid and
...
plasmid-specific portion was extracted (see below, pQBR47p).
...
The whole contig was used for downstream analysis, and the
plasmid-specific portion was extracted (see below, pQBR106p).
...
It was downloaded and the two contigs were concatenated for analysis
(pQBR44d).
...
Note the location of the concatenation between the contigs for the draft
sequences:

- pQBR11: around 110793, owing to the presence of a 302 bp repeat. A
  resolved assembly for pQBR11 could be generated by orienting the
  contigs with reference to pQBR103 and joining them.
- pQBR44d: positions 41265-41266 and 92628-92629.
- pQBR50d: positions 361069-361070 and 480080-480081.
...
plasmid-specific portion. The plasmid-specific portion is flanked by
copies of Tn6290 in the complete assembly. The breakpoint for this
assembly is therefore at 279596.

...
The plasmids were sequenced alongside *P. putida* UWC1, a derivative of
*P. putida* KT2440. Initial Illumina sequencing was performed by
MicrobesNG. These data were analysed in an attempt to generate closed
plasmid sequences as follows:

1.  Map all reads against the *P. putida* KT2440 genome using `bwa-mem`
    0.7.17-r1188.
...
visualising the assembly graphs in Bandage and identifying and
...
‘Groups’ are based on the RFLP analyses of [Lilley et
al. 1996](dx.doi.org/10.1111/j.1574-6941.1996.tb00320.x).

#### Group I (pQBR103)-like

- pQBR4 — ONT sequencing in pool 2
- pQBR5 — ONT sequencing in pool 1
- pQBR11 — ONT sequencing in pool 5, but this doesn’t seem to have
  resolved in the sample.
...
- pQBR103 — closed by Illumina alone. This is resequenced: pQBR103 was
  previously sequenced ([Tett et
  al. 2007](dx.doi.org/10.1038/ismej.2007.47))
...
- pQBR55 — closed by Illumina alone. This is resequenced: pQBR55 was
  previously sequenced ([Hall et
  al. 2007](dx.doi.org/10.1111/1462-2920.12901))
...
- pQBR57 — ONT sequencing in pool 2. This is resequenced: pQBR57 was
  previously sequenced ([Hall et
  al. 2007](dx.doi.org/10.1111/1462-2920.12901))
...
- pQBR1 — Illumina indicated no plasmid present. Instead, and in
  contrast to the ~70-kb plasmid referred to by Lilley et al. (1996),
  the assembly of unmapped reads shows a 9.9 kb sequence that has
  integrated into the chromosome. This suggests that the stock strain
  has lost the plasmid but kept the transposon, as described in various
  lab studies with other pQBR plasmids. We were not able to detect
  conjugation in the lab, further supporting our inference that the
  plasmid has been lost.

#### Others

- pQBR8 — Illumina inducated low coverage for regions of interest,
  suggesting that the plasmid was in the process of being lost when the
  sample was sequenced. We were not able to detect conjugation in the
  lab, further supporting our inference that the plasmid has been lost.
- pQBR58 — Illumina assembly of unmapped reads gave one non-circular
  contig, 73.7 kb. Was not included in ONT sequencing.
...
assembled by Illumina. The long-read contigs were extracted and named.

Some manual analysis was required for the following, possibly due to
sequence loss, possibly due to chromosome integration, possibly both.
...
contain the same duplicated element (possibly a transposon) but remain
separate replicons, or the plasmid has integrated into the chromosome,
flanked by copies of the duplication.

***To investigate:*** look for long reads that map both to plasmid and
chromosome.
...
replicons that both resembled Group I and Group III plasmids.

<figure>
<img
src="/Users/jpjh/Library/Application%20Support/typora-user-images/Screenshot%202024-02-20%20at%2017.52.36.png"
alt="Screenshot 2024-02-20 at 17.52.36" />
<figcaption aria-hidden="true">Screenshot 2024-02-20 at
17.52.36</figcaption>
</figure>
...
present, of which only one remains in our sample.

#### pQBR47

This sequence didn’t fully resolve owing to a duplicate sequence in the
chromosome, and the sequence is assembled as a single contig. Perhaps
the plasmid has also integrated into the chromosome.
...
<figure>
<img
src="/Users/jpjh/Library/Application%20Support/typora-user-images/Screenshot%202024-04-30%20at%2015.39.05.png"
alt="Screenshot 2024-04-30 at 15.39.05" />
<figcaption aria-hidden="true">Screenshot 2024-04-30 at
15.39.05</figcaption>
</figure>

Like with pQBR43 and pQBR50 this plasmid appears to have integrated into
the chromosome, and lost a portion of the sequence relative to pQBR103.
...
Similarly, this sequence didn’t fully resolve, possibly due to loss of
(part of) the plasmid.
